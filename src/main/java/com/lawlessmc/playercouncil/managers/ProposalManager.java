...
public class ProposalManager {
    ...
    private int getRequiredVotes(Proposal p) {
        // Ban-type proposals store the frozen threshold in value at create time
        ...
        switch (p.getType()) {
            ...
            case PARDON, REPARDON -> Bukkit.getScheduler().runTask(plugin, () -> {
                ...
                plugin.getBanVoteManager().advanceAfterSuccess(p.getType(), targetUuid,
                        off.getName() != null ? off.getName() : p.getTarget());
                // Overturned ban → target cannot propose bans for N days
    ...
    private void dispatchBan(String playerName, String reason) {
        ...
        if ("vanilla".equalsIgnoreCase(provider)) {
            ...
            // smartban (default): /ban <player> <reason...>
    ...
    /**
     * Apply a gamerule to every loaded world (boolean or integer rules).
     */
    private void applyGameRuleAllWorlds(String ruleName, String valueStr) {
        ...
        // Also try original casing variants for older servers
        ...
        for (var e : GAMERULE_ALIASES.entrySet()) {
            if (e.getValue().equalsIgnoreCase(modern)) {
                candidates.add(e.getKey()); // old name
        ...
        // Console fallback — works for 26.x registry names and older servers
        for (String cand : candidates) {
    ...
    /**
     * Apply any pending enable/disable actions from a previous successful vote.
     * Called once during plugin enable, after other plugins have loaded.
     */
    ...
    /**
     * Old camelCase (pre-1.21.11) → new snake_case (1.21.11 / 26.x).
    ...
    /** Preferred modern name for apply; falls back to normalized input. */
    ...
    /** All known rule name candidates (aliases + live API), lowercase. */
    ...
    /**
     * Best fuzzy match for a typed gamerule. Returns null if nothing is close enough.
     * Prefers alias modern names when tied.
     */
    public String findClosestGamerule(String input) {
        ...
        // Also try without underscores vs with
        ...
        for (String candidate : allKnownGameruleNames()) {
            ...
            // Also compare stripped underscores (keepinventory ≈ keep_inventory)
            ...
            if (d < bestDist) {
                ...
                // Prefer the resolved modern form
        ...
        // Allow more slack for longer names
    ...
    /**
     * Resolve input to a canonical gamerule name, using aliases and fuzzy match.
     * Returns null if nothing acceptable is found.
     */
    public String resolveGameruleInput(String input) {
        ...
        // Last resort: accept plausible identifier as-is (console may still apply it)
