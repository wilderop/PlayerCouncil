...
public class ProposalManager {
    ...
    public void createProposal(Player proposer, Proposal.Type type, String target, String value) {
        createProposal(proposer, type, target, value, null);
    }
    ...
    private static String sanitizeReason(String reason) {
        ...
        if (r.length() > 200) r = r.substring(0, 200).trim();
        return r;
    }
    ...
    private void execute(Proposal p) {
        switch (p.getType()) {
            case BAN, REBAN -> Bukkit.getScheduler().runTask(plugin, () -> {
                ...
                String storedReason = buildStoredBanReason(p);
                String kickMessage = buildCouncilBanMessage(p, uuid, storedReason);
                dispatchBan(p.getTarget(), storedReason, kickMessage);
                // Track for automated council review on join
            ...
            case PARDON, REPARDON -> Bukkit.getScheduler().runTask(plugin, () -> {
                ...
                plugin.getBanReviewManager().recordUnban(targetUuid,
                        off.getName() != null ? off.getName() : p.getTarget());
    ...
    private String buildStoredBanReason(Proposal p) {
        String proposerName = resolveName(p.getProposer());
        String stated = p.getReason();
        if (stated != null && !stated.isBlank()) {
            return sanitizeReason(stated) + " (council: " + proposerName + ")";
        }
        String kind = p.getType() == Proposal.Type.REBAN ? "Council re-ban" : "Council ban";
        return kind + " by " + proposerName;
    }

    private String buildCouncilBanMessage(Proposal p, UUID bannedUuid, String storedReason) {
        int pardonVotes = p.getType() == Proposal.Type.REBAN
                ? plugin.getConfig().getInt("voting.repardon", 8)
                : plugin.getConfig().getInt("voting.pardon", 2);

        String proposerName = resolveName(p.getProposer());
        java.util.List<String> others = new java.util.ArrayList<>();
        for (UUID id : plugin.getCouncilManager().getCouncilMembers()) {
            if (id.equals(bannedUuid) || id.equals(p.getProposer())) continue;
            String n = resolveName(id);
            if (n != null && !n.isBlank()) others.add(n);
        }
        others.sort(String.CASE_INSENSITIVE_ORDER);
        String othersLine = others.isEmpty()
                ? "(none currently seated besides the proposer)"
                : String.join(", ", others);

        return "You have been banned by a council member.\n"
                + "Reason: " + storedReason + "\n"
                + "There is an appeals process: " + pardonVotes
                + " council member(s) can vote to pardon you.\n"
                + "Banned by: " + proposerName + "\n"
                + "Other council members who can vote to pardon you: " + othersLine;
    }

    private static String resolveName(UUID uuid) {
        if (uuid == null) return "Unknown";
        OfflinePlayer off = Bukkit.getOfflinePlayer(uuid);
        if (off.getName() != null && !off.getName().isBlank()) return off.getName();
        return uuid.toString().substring(0, 8);
    }

    private void dispatchBan(String playerName, String storedReason, String kickMessage) {
        if (storedReason == null || storedReason.isBlank()) {
            storedReason = "Council ban";
        }
        if (kickMessage == null || kickMessage.isBlank()) {
            kickMessage = storedReason;
        }
        ...
        String reasonForCmd = storedReason.replace('\n', ' ').replace('\r', ' ').trim();
        String cmd = template
                .replace("{player}", playerName)
                .replace("{reason}", reasonForCmd);

        if ("vanilla".equalsIgnoreCase(provider)) {
            OfflinePlayer off = Bukkit.getOfflinePlayer(playerName);
            if (off.getName() != null) {
                Bukkit.getBanList(org.bukkit.BanList.Type.NAME).addBan(
                        off.getName(), storedReason, (java.util.Date) null, "PlayerCouncil");
            }
            Player online = Bukkit.getPlayerExact(playerName);
            if (online != null) {
                online.kick(net.kyori.adventure.text.Component.text(kickMessage));
            }
            plugin.getLogger().info("Vanilla ban applied to " + playerName + ": " + reasonForCmd);
        } else {
        ...
        plugin.getDatabaseManager().log("Banned " + playerName + " via " + provider + ": " + reasonForCmd);
    }
        ...
        for (var world : Bukkit.getWorlds()) {
            try {
                for (GameRule<?> rule : GameRule.values()) {
                    if (rule.getName().equalsIgnoreCase(modern) || rule.getName().equalsIgnoreCase(ruleName)) {
                        ...
                        Bukkit.dispatchCommand(Bukkit.getConsoleSender(),
                                "execute in " + world.getKey() + " run gamerule " + modern + " " + valueStr);
                        break;
                    }
                }
            } catch (Exception e) {
                plugin.getLogger().warning("Failed gamerule on " + world.getName() + ": " + e.getMessage());
            }
        }
    ...
    private static java.util.Map<String, String> buildGameruleAliases() {
        ...
        String[][] pairs = {
            {"doInsomnia", "spawn_phantoms"}, {"doDaylightCycle", "advance_time"},
            {"doWeatherCycle", "advance_weather"}, {"doMobSpawning", "spawn_mobs"},
            {"doMobLoot", "mob_drops"}, {"doTileDrops", "block_drops"},
            {"doEntityDrops", "entity_drops"}, {"keepInventory", "keep_inventory"},
            {"mobGriefing", "mob_griefing"}, {"naturalRegeneration", "natural_health_regeneration"},
            {"doImmediateRespawn", "immediate_respawn"}, {"doLimitedCrafting", "limited_crafting"},
            {"doPatrolSpawning", "spawn_patrols"}, {"doTraderSpawning", "spawn_wandering_traders"},
            {"doWardenSpawning", "spawn_wardens"}, {"doVinesSpread", "spread_vines"},
            {"disableRaids", "raids"}, {"announceAdvancements", "show_advancement_messages"},
            {"commandBlockOutput", "command_block_output"}, {"logAdminCommands", "log_admin_commands"},
            {"showDeathMessages", "show_death_messages"}, {"sendCommandFeedback", "send_command_feedback"},
            {"reducedDebugInfo", "reduced_debug_info"}, {"spectatorsGenerateChunks", "spectators_generate_chunks"},
            {"spawnRadius", "respawn_radius"}, {"maxEntityCramming", "max_entity_cramming"},
            {"randomTickSpeed", "random_tick_speed"}, {"playersSleepingPercentage", "players_sleeping_percentage"},
            {"maxCommandChainLength", "max_command_sequence_length"}, {"maxCommandForkCount", "max_command_forks"},
            ...
            {"fallDamage", "fall_damage"}, {"fireDamage", "fire_damage"},
            {"drowningDamage", "drowning_damage"}, {"freezeDamage", "freeze_damage"}, {"pvp", "pvp"},
        };
    ...
    public String findClosestGamerule(String input) {
        ...
        if (GAMERULE_ALIASES.containsKey(key)) return resolveGameruleName(key);
        for (GameRule<?> rule : GameRule.values()) {
            if (rule.getName().equalsIgnoreCase(key)) return resolveGameruleName(rule.getName());
        }
        String keyNoUnderscore = key.replace("_", "");
        String best = null;
        int bestDist = Integer.MAX_VALUE;
        for (String candidate : allKnownGameruleNames()) {
            int d = Math.min(levenshtein(key, candidate), levenshtein(keyNoUnderscore, candidate.replace("_", "")));
